...
class VOCSegmentation(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transforms is not None:
            ...
            # Tạo label classification từ mask (target)
            target_np = np.array(target)
            class_labels = torch.zeros(104)  # 104 lớp
            unique_classes = np.unique(target_np)  # Các lớp có trong mask
            for cls in unique_classes:
                if cls != 255:  # Bỏ ignore index
                    class_labels[cls] = 1.0  # Binary: 1 nếu lớp tồn tại

            return (img, aux), target, class_labels  # Thêm class_labels

            # return (img, aux), target # Nhóm 2 ảnh vào một tuple
        
        return img, target, torch.zeros(104)
    ...
    def collate_fn(batch):
        images_tuple, targets, class_labels = list(zip(*batch))  # Thêm class_labels
        ...
        batched_class_labels = torch.stack(class_labels)
        return (batched_imgs, batched_aux), batched_targets, batched_class_labels
    # def collate_fn(batch):
